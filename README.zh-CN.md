...
- 可从“设置”扫描并单向导入 Obsidian Vault，兼容 YAML 属性、`[[双链]]` 与本地附件，并在导入前预览冲突
...
打开应用的“设置 → 备份”，选择另一块磁盘或其他独立目录。数据变化会被合并处理，默认最多每小时自动生成一个带时间戳的逻辑完整快照；也可以随时手动备份。未变化文件优先使用 APFS Clone-on-Write，无法克隆时复用上一快照或复制，因此图片、视频不会在每次备份时无条件完整复制。快照包含用户配置、SQLite 数据库、文章、草稿、动态、活动记录、回收站和媒体文件；`.leon-book.lock` 锁文件不会被复制。
...
文章 Markdown（含 YAML Properties）和 `media/` 下的普通媒体文件是资料库的权威数据源；SQLite 保存可由这些文件重新扫描得到的文章索引，并继续承载评论、版本、收藏等非文章结构化数据。首次启动时，已有 JSON 数据会自动迁移；之后通过 macOS FSEvents 合并处理外部 Markdown 的新增、修改、移动和删除，只读取变更路径，并每 15 分钟执行一次低频完整校验。

导入 Obsidian Vault 时，应用只读取 Vault，并把确认后的文章和附件复制到当前用户工作空间；不会监控或回写原 Vault。复制后的 Markdown/附件成为当前工作空间的权威副本，已有相同 slug 的文章会被跳过。
