...
if let articleViews = try? String(contentsOfFile: sourcePath("ArticleViews.swift"), encoding: .utf8) {
    ...
    expect(articleViews.contains("MarkdownArticleBody(\n                            body: article.body,"), "article reader should render article Markdown instead of showing its source")
    ...
    expect(articleViews.contains("MarkdownPreview(\n                    markdown: model.editor.body,"), "article editor should render Markdown live beside its input")
