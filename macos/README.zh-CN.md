...
文章、草稿、设置、图片和视频都保存在本地文件系统中。应用不会启动 Node.js、浏览器或本地 HTTP 服务。
...
`Notebook36` 是当前 SwiftPM target 的内部名称；应用对外显示名称为 `leon-book`。
...
4. `~/Library/Application Support/Notebook 36/`（旧版目录存在时）
5. `~/Library/Application Support/leon-book/`

默认用户为 `leon`。每位用户都有独立的 `workspaces/<user-id>` 目录。升级到多用户结构时，根目录中已有的文章、草稿、媒体、动态和活动记录会自动迁移到 `leon` 工作空间。

如果安装了多个 macOS SDK，可以使用 `LEON_BOOK_SDK_PATH` 指定打包所用的 SDK。旧配置 `NOTEBOOK36_SDK_PATH` 仍然受支持：
...
```text
macos/
├── Sources/Notebook36/      # SwiftUI 应用源码
├── Checks/Notebook36Checks/  # 原生检查
